...
function fixDirectoryImports(code, filePath, cfg) {
  ...
  updated = updated.replace(
    /(from\s*["'])(@\w+\/[^\/"']+\/[^"']*?)(?<!\.js|\.d\.ts)(["'])/g,
    (match, prefix, importPath, suffix) => {
      // @qvac/core resolves its subpaths through an `exports` map; appending
      // `.js` to an export key (e.g. `@qvac/core/surface`) would break it.
...
async function processFile(filePath, cfg) {
  ...
  const replaceExportFrom = (code) =>
    code.replace(/(export\s+[^;]*?from\s*["'])(@\/[^"']+)(["'])/g, (m, p1, spec, p3) => {
      const rel = resolveAliasToRelative(spec, filePath, cfg)
      return `${p1}${rel}${p3}`
    })

  const replaceDynamicImport = (code) =>
    code.replace(/(import\s*\(\s*["'])(@\/[^"']+)(["']\s*\))/g, (m, p1, spec, p3) => {
      const rel = resolveAliasToRelative(spec, filePath, cfg)
      return `${p1}${rel}${p3}`
    })
