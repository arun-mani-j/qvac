...
const commonModelConfigSchema = z.object({
  ...
  requestId: z
    ...
    .optional()
    .describe(
      'Stable identifier for this in-flight load, generated by the caller at call time. Optional — core falls back to a generated id when the field is missing. Exposed on the decorated promise so callers can target this load with `cancel({ requestId })`.'
    )
})
 ...
 * Users should use model constants from @qvac/core instead of raw URLs.
